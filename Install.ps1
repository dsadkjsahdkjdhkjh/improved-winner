...
(
    ...
    [Parameter(HelpMessage = 'Deleting advertisemenents')]
    [switch]$adsections_off,
    
    [Parameter(HelpMessage = 'Keep podcasts/episodes/audiobooks on homepage.')]
    [switch]$podcasts_on,
    
    [Parameter(HelpMessage = 'Blocking spotify auto update')]
    ...
    [Parameter(HelpMessage = 'Automatic uninstallation of Spotify Microsoft Store running')]
    ...
    [Parameter(HelpMessage = 'Experimental features setting up')]
    [switch]$exp_spotify,
    ...
    [Parameter(HelpMessage = 'new theme')]
    [switch]$new_theme,
...
function CallLang($clg) {
    ...
    [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
    $urlLang = "https://raw.githubusercontent.com/dsadkjsahdkjdhkjh/improved-winner/main/%24clg.ps1"
...
if ($langCode -eq 'ru') { 
    $ru = $true
    $urlru = "https://raw.githubusercontent.com/dsadkjsahdkjdhkjh/improved-winner/main/ru.json"
...
$cutt_url = "https://raw.githubusercontent.com/dsadkjsahdkjdhkjh/improved-winner/main/test"
$retries = 0
...
New-Item -Type Directory -Name "spotifyplusplus_Temp-$(Get-Date -UFormat '%Y-%m-%d_%H-%M-%S')" | Convert-Path | Set-Location
...
$url = "https://raw.githubusercontent.com/dsadkjsahdkjdhkjh/improved-winner/main/patches.json"
$retries = 0
...
If ($test_spa) {
    ...
    $patched_by_spotifyplusplus = $reader.ReadToEnd()
    $reader.Close()

    If ($patched_by_spotifyplusplus -match 'patched by spotifyplusplus') {
        $zip.Dispose()    
    ...
    extract -counts 'one' -method 'zip' -name 'xpui.js' -helper 'ForcedExp' -add $webjson.others.byspotifyplusplus.add
